...
1. 先把本仓库部署到 Cloudflare Pages 或 Vercel（见下一节），拿到你的域名，例如 `laozig-statusshow.pages.dev`。
2. 把下面按钮链接里的 `YOUR-PROJECT.pages.dev` 换成**你自己的域名**，然后点击即可在主控里添加本主题：

<a href="https://dash.nodeget.com/#/dashboard/theme-management?add=https://YOUR-PROJECT.pages.dev">
  <img src="https://dash.nodeget.com/deploy-button.png" alt="deploy button" width="230px" />
</a>

```text
https://dash.nodeget.com/#/dashboard/theme-management?add=https://YOUR-PROJECT.pages.dev
```

3. 部署上线后，建议把 [nodeget-theme.json](nodeget-theme.json) 的 `dist_page` 填成你的域名（如 `https://laozig-statusshow.pages.dev`），页脚的「检查更新 / 下载最新版」才会指向你自己的产物。

> 这是你**自己的**一键部署：按钮指向你的域名而非上游官方站点，添加进主控后拉取的是你二开的主题。
